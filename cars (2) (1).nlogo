...
[
  ...
  ;; the real-world equivalent on a tick (in years)
...
turtles-own
[
	lifespan							;; how long a car will go
  age
  production-carbon
  annual-carbon
]
...
to setup-globals
  ...
  set total-emissions 0
	...
	;; how many ticks in one year
  set tick-duration 365.25
...
to setup-cars
  ...
  let bolt-data array:from-list [70 1 6]
  let spark-data array:from-list [20 50 10]
  let tesla-data array:from-list [100 0 5]
  let mazda-data array:from-list [5 30 15]
  ...
  [
    ...
    [
      ...
      set lifespan ( (array:item spark-data (2)) * tick-duration )
    ]
    [
      set production-carbon array:item mazda-data 0
      set annual-carbon array:item mazda-data 1
      set lifespan ( (array:item mazda-data (2)) * tick-duration )
    ]
  ...
  [
    ...
    [
      ...
      set lifespan ( (array:item bolt-data (2)) * tick-duration )
    ]
    [
      set production-carbon array:item tesla-data 0
      set annual-carbon array:item tesla-data 1
      set lifespan ( (array:item tesla-data (2)) * tick-duration )
    ]
...
to drive  ;; turtle procedure
  ifelse random 10 < 8 ;; 80% chance to go for a drive
  [
    fd 5
    set age (age + 1)
  ]
  ...
  set total-emissions (total-emissions + (annual-carbon / tick-duration))
end
