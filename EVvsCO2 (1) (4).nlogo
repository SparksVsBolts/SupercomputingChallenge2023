...
to setup-globals
  set car-size 4
  set total-emissions 0
  set total-emissions-gas 0
  set total-emissions-ev 0
  
  set cars-remaining ( gas-count + ev-count )
end
...
to setup-cars
  ...
  ; first item is inital co2 prod
  ; second item is annual co2 prod
  ; third item is lifespan in years  
  
  ; using 14,263 miles driven per year
  
  let bolt-data array:from-list [0 1312 8]
  ; Chevy plans to make their factories carbon neutral
  ; approx 1312 kg per year | 92 grams of CO2 per mile
  ; 8 years
  
  let spark-data array:from-list [0 3837 12]
  ; Chevy plans to make their factories carbon neutral
  ; approx 3837 kg per year | 269 grams of C02 per mile
  ; ranges from 10 to 14 years | 150,000 to 200,000 miles
  
  let tesla-data array:from-list [9200 1859 28]
  ; range between 2400 kg and 16,000 kg
  ; approx 1859 kg per year | 0.81 kg per 10 km
  ; between 21 and 35 years of operation
  
  let mazda-data array:from-list [0 3191 18]
  ; Mazda plans to make their factories carbon neutral
  ; approx 3191 kg per year | 139 kg per km
  ; ranges from 14 to 21 years | 200,000 to 300,000 miles
  
	create-turtles (gas-count)
  [
    setxy random-xcor random-ycor         ; randomize turtle locations
    set size car-size	                        	; easier to see
    set color red
    set age 0
    set type-of-car "gas"
    ;; 
    set heading (round (random 360 / 90) ) * 90
    
    ;; give each car data
    ifelse gas-model = "Chevy Spark"
    [
      set production-carbon array:item spark-data 0
      set annual-carbon array:item spark-data 1
      set lifespan ( (array:item spark-data (2)) * ticks-per-year )
    ]
    [
      set production-carbon array:item mazda-data 0
      set annual-carbon array:item mazda-data 1
      set lifespan ( (array:item mazda-data (2)) * ticks-per-year )
    ]
    
    
    set total-emissions (total-emissions + production-carbon)
    set total-emissions-gas (total-emissions-gas + production-carbon)

  ]
  create-turtles (ev-count)
  [
    setxy random-xcor random-ycor         ; randomize turtle locations
    set size car-size                           	; easier to see\
    set color blue
    ...
    ifelse electric-model = "Chevy Bolt"
    [
      set production-carbon array:item bolt-data 0
      set annual-carbon array:item bolt-data 1
      set lifespan ( (array:item bolt-data (2)) * ticks-per-year )
    ]
    [
      set production-carbon array:item tesla-data 0
      set annual-carbon array:item tesla-data 1
      set lifespan ( (array:item tesla-data (2)) * ticks-per-year )
    ]
    
    set total-emissions (total-emissions + production-carbon)
    set total-emissions-ev (total-emissions-ev + production-carbon)
  ]
...
to drive  ;; turtle procedure
  ifelse random 10 < 5 ;; 50% chance to go for a drive
  [
    fd 5
    set age (age + 1)
  ]
  [
    ifelse random-float 2 > 1
    [ rt 90 ]
    [ rt -90]
	  set age (age + 0.25)
  ]
  
  ifelse type-of-car = "gas"
  [
    set total-emissions-gas (total-emissions-gas + (annual-carbon / ticks-per-year))
	]
  [
    set total-emissions-ev (total-emissions-ev + (annual-carbon / ticks-per-year))
  ]
  
  set total-emissions (total-emissions + (annual-carbon / ticks-per-year))
end
...
false
false
"" "if ticks / ticks-per-year > plot-x-max\n[\n  set-plot-x-range 0 (plot-x-max + 1)\n]\n\nif total-emissions > plot-y-max\n[\n  set-plot-y-range 0 (plot-y-max + 50000)\n]"
PENS
"Gas Vehicle Emissions" 1 0 -2674135 true "" "if ticks mod ticks-per-year < 1\n[\n  plotxy (ticks / ticks-per-year) total-emissions-gas\n]"
"Electric Vehicle Emissions" 1 0 -13345367 true "" "if ticks mod ticks-per-year < 1\n[\n  plotxy (ticks / ticks-per-year) total-emissions-ev\n]"
@#$#@#$#@
## WHAT IS IT?
